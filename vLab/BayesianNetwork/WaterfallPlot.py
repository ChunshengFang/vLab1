'''
This is a walkaround version to generate waterfall plot without known bug in SHAP library
Full credit to https://github.com/slundberg/shap
...
from .Util import safe_isinstance, format_value
from .colors import _colors as colors
...
# of the bar...
def waterfall(shap_values, max_display=10, show=True, tick_mark_expectation="\n$E[f(X)]$", tick_mark_prediction="$f(x)$", time=None, save=None):
    ...
    pl.yticks(ytick_pos, yticklabels[:-1] + [l.split('=')[-1] for l in yticklabels[:-1]], fontsize=18)
    ...
    ax2.set_xticklabels([tick_mark_expectation, "                   \n     $= " + format_value(base_values, "%0.02f") + "$"], fontsize=15, ha="center")
    ...
    ax3.set_xticklabels([tick_mark_prediction, "                   $ = " + format_value(fx, "%0.02f") + "$"], fontsize=15, ha="center")
    ...
    if save:
        pl.title('{time}h'.format(time=time))
        pl.tight_layout()
        pl.savefig(save)
    ...
    if show:
        ...
        try:
            ...
            print("Deprecated for python version > 3.7")
        pl.clf()
    else:
        return pl.gcf()
...
def waterfall_legacy(expected_value, shap_values=None, features=None, feature_names=None, max_display=10, show=True):
    ...
    ax2.set_xticklabels(["\n$E[f(X)]$", "\n$ = " + format_value(expected_value, "%0.03f") + "$"], fontsize=15,
                        ha="left")
    ax2.spines['right'].set_visible(False)
    ax2.spines['top'].set_visible(False)
    ax2.spines['left'].set_visible(False)
    ...
    ax3.set_xticklabels(["$f(x)$", "$ = " + format_value(fx, "%0.03f") + "$"], fontsize=15, ha="left")
    tick_labels = ax3.xaxis.get_majorticklabels()
    tick_labels[0].set_transform(
        tick_labels[0].get_transform() + matplotlib.transforms.ScaledTranslation(-10 / 72., 0, fig.dpi_scale_trans))
    tick_labels[1].set_transform(
